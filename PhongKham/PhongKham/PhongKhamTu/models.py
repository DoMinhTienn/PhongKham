...
class DonKham(BaseModel):
    dang_ky = models.OneToOneField(DangKy, on_delete=models.CASCADE, related_name='don_kham')
    Trieu_chung = RichTextField()
    ket_luan = RichTextField()
    toa_thuoc = models.ManyToManyField(Thuoc, through='ToaThuoc', related_name='don_kham')

class ToaThuoc(BaseModel):
    donkham = models.ForeignKey(DonKham, on_delete=models.CASCADE, related_name='toathuoc')
    thuoc = models.ForeignKey(Thuoc, on_delete=models.CASCADE, related_name='toathuoc')
